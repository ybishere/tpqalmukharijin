...
@section('title', 'In Memoriam')
@section('page_title', 'In Memoriam')
@section('breadcrumb')
...
<div class="flex items-center justify-between mb-6">
    <div>
        ...
        <p class="text-xs text-stone-400 mt-0.5">{{ $memoriam->count() }} tokoh</p>
    </div>
    <a href="{{ route('admin.memoriam.create') }}"
        ...
        <tbody class="divide-y divide-stone-50">
            ...
            <tr class="hover:bg-stone-50/50 transition-colors">
                <td class="px-5 py-3.5">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            @if($item->biografi)
                            <p class="text-stone-400 text-[11px] mt-0.5">{{ Str::limit($item->biografi, 50) }}</p>
                            @endif
                        </div>
                    </div>
                </td>
                ...
                <td class="py-3.5 text-center">
                    <span class="text-xs font-semibold text-stone-500">{{ $item->quotes_count }}</span>
                </td>
                <td class="py-3.5 text-center">
                    <span class="text-xs font-semibold text-stone-500">{{ $item->photos_count }}</span>
                </td>
                <td class="py-3.5 text-center">
                    <span class="text-xs text-stone-400">{{ $item->urutan }}</span>
                </td>
                <td class="px-5 py-3.5">
                    <div class="flex items-center justify-end gap-2">
                        <a href="{{ route('admin.memoriam.edit', $item) }}"
                        ...
                        <form method="POST" action="{{ route('admin.memoriam.destroy', $item) }}"
