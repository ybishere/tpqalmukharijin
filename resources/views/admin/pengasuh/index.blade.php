...
@section('title', 'Pengasuh & Pengajar')
@section('page_title', 'Pengasuh & Pengajar')
@section('breadcrumb')
...
<div class="flex items-center justify-between mb-6">
    <div>
        ...
        <div class="flex items-center gap-3 mt-1">
            <span class="text-[11px] font-semibold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full">{{ $totalPengasuh }} Pengasuh</span>
            <span class="text-[11px] font-semibold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full">{{ $totalPengajar }} Pengajar</span>
        </div>
    </div>
    ...
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-stone-50">
            ...
            <tr class="hover:bg-stone-50/50 transition-colors">
                <td class="px-5 py-3.5">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            @if($item->quotes)
                            <p class="text-stone-400 text-[11px] mt-0.5 italic truncate max-w-[200px]">"{{ Str::limit($item->quotes, 50) }}"</p>
                            @endif
                        </div>
                    </div>
                </td>
                <td class="py-3.5 text-xs text-stone-500">{{ $item->jabatan ?? '-' }}</td>
                <td class="py-3.5 text-center">
                    @if($item->status === 'Pengasuh')
                ...
                <td class="py-3.5 text-center text-xs text-stone-400">{{ $item->urutan }}</td>
                <td class="px-5 py-3.5">
                    <div class="flex items-center justify-end gap-2">
                        <a href="{{ route('admin.pengasuh.edit', $item) }}"
                        ...
                        <form method="POST" action="{{ route('admin.pengasuh.destroy', $item) }}"
