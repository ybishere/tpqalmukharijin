...
@section('page_title', 'Manajemen Donasi')
...
<div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
    <div class="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
        ...
        <p class="text-2xl font-extrabold text-emerald-600">Rp {{ number_format($totalMasuk, 0, ',', '.') }}</p>
    </div>
    <div class="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
        <p class="text-xs text-gray-400 font-medium uppercase tracking-wider mb-1">Total Transaksi</p>
        <p class="text-2xl font-extrabold text-gray-800">{{ $donasis->total() }}</p>
    </div>
    <div class="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
        <p class="text-xs text-gray-400 font-medium uppercase tracking-wider mb-1">Menunggu Konfirmasi</p>
        <p class="text-2xl font-extrabold text-amber-500">{{ $totalPending }}</p>
    </div>
</div>

{{-- Filter + Tambah --}}
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm mb-5 p-4">
    <form method="GET" action="{{ route('admin.donasi.index') }}" class="flex flex-wrap gap-3 items-end">
        ...
        <div class="flex gap-2">
            ...
            @if(request()->hasAny(['search','status','program_id']))
                <a href="{{ route('admin.donasi.index') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-600 font-semibold px-4 py-2 rounded-xl text-sm transition-colors">
                    Reset
                </a>
            @endif
        </div>
...
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    <td class="px-5 py-4 font-medium text-gray-800">{{ $donasi->nama_donatur }}</td>
                    <td class="px-5 py-4 text-gray-500 max-w-[160px]">
                        <span class="truncate block">{{ $donasi->program->nama_program ?? '-' }}</span>
                    </td>
                    ...
                    <td class="px-5 py-4">
                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold
                            {{ $donasi->metode === 'qris' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700' }}">
                    ...
                    <td class="px-5 py-4 text-gray-400 text-xs">{{ $donasi->created_at->format('d/m/Y H:i') }}</td>
                    <td class="px-5 py-4">
                        <div class="flex items-center gap-1.5">
                            ...
                            <form method="POST" action="{{ route('admin.donasi.destroy', $donasi) }}"
    ...
    @if ($donasis->hasPages())
    <div class="px-5 py-4 border-t border-gray-100">{{ $donasis->links() }}</div>
    @endif
</div>
