...
@section('page_title', 'Edit Penggunaan Dana')
...
<div class="max-w-2xl">
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div class="p-6">
            <form method="POST" action="{{ route('admin.penggunaan.update', $penggunaan) }}" enctype="multipart/form-data" class="space-y-5">
                @csrf @method('PUT')
                ...
                <div>
                    ...
                    <select name="program_id"
                        ...
                        @foreach ($programs as $p)
                            <option value="{{ $p->id_program }}" {{ old('program_id', $penggunaan->program_id) == $p->id_program ? 'selected' : '' }}>
                                {{ $p->nama_program }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <textarea name="uraian" rows="4"
                        class="w-full border {{ $errors->has('uraian') ? 'border-red-400 bg-red-50' : 'border-gray-300' }} rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 transition-all resize-none">{{ old('uraian', $penggunaan->uraian) }}</textarea>
                ...
                <div>
                    ...
                    @if ($penggunaan->bukti_foto)
                        <img src="{{ Storage::url($penggunaan->bukti_foto) }}" class="w-32 h-24 object-cover rounded-xl mb-3">
